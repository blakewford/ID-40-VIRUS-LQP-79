...
void drawDoor()
{
  if (exitDoor.active)
  {
    ...
    for (byte x = 0; x < 2; x++)
    {
      for (byte y = 0; y < 2; y++)
      {
        sprites.drawErase(exitDoor.x - mapPositionX + (x * 8), exitDoor.y - mapPositionY + (y * 8), virus::getBitmap(virus::EXITOPENMASK_IMAGE), 0);
        sprites.drawSelfMasked(exitDoor.x - mapPositionX + (x * 8), exitDoor.y - mapPositionY + (y * 8), virus::getBitmap(virus::EXITOPEN_IMAGE), exitDoor.frame + 4 * exitDoor.orientation);
      }
    }
  }
  else
  {
    sprites.drawErase(exitDoor.x - mapPositionX, exitDoor.y - mapPositionY, virus::getBitmap(virus::EXITCLOSEDMASK_IMAGE), 0);
    sprites.drawSelfMasked(exitDoor.x - mapPositionX, exitDoor.y - mapPositionY, virus::getBitmap(virus::EXITCLOSED_IMAGE), exitDoor.orientation);
  }
}
