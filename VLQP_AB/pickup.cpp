...
void drawPickups()
{
  for (byte id = 0; id < PICKUP_MAX; id++)
  {
    ...
    if ((pickups[id].type > PICKUP_TYPE_INACTIVE) && pickups[id].isVisible) sprites.drawPlusMask(pickups[id].x - mapPositionX, pickups[id].y - mapPositionY, virus::getBitmap(virus::COLLECTABLE_IMAGE), pickups[id].frame + (6 * (pickups[id].type - 1)));
  }
}
